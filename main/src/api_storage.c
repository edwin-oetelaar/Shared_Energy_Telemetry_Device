...
esp_err_t api_storage_save_credentials(const char *client_id, const char *client_secret)
{
    ...
    nvs_handle_t handle;
    esp_err_t err = nvs_open("api_credentials", NVS_READWRITE, &handle);
    if (err != ESP_OK) {
...
esp_err_t api_storage_load_credentials(char *client_id, size_t client_id_len, char *client_secret, size_t client_secret_len)
{
    ...
    nvs_handle_t handle;
    esp_err_t err = nvs_open("api_credentials", NVS_READONLY, &handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open NVS handle: %s", esp_err_to_name(err));
        return err;
    }
    ...
    err = nvs_get_str(handle, "client_id", NULL, &required_client_id_len);
    ...
    err = nvs_get_str(handle, "client_secret", NULL, &required_client_secret_len);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
    ...
    err = nvs_get_str(handle, "client_id", client_id, &required_client_id_len);
    if (err != ESP_OK) {
    ...
    err = nvs_get_str(handle, "client_secret", client_secret, &required_client_secret_len);
    if (err != ESP_OK) {
...
esp_err_t api_storage_clear_credentials(void)
{
    ...
    err = nvs_erase_key(handle, "client_id");
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
    ...
    err = nvs_erase_key(handle, "client_secret");
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
